...
class Installer
{
	...
	public $builtinModules = array('lib', 'db', 'rdf', 'framework', 'auth', 'install', 'setup', 'gendoc', 'testsuite', 'login', 'silk', 'log', 'views');
	...
	protected function checkIndex()
	{
		...
		else
		{
			...
			fwrite($f, '/* You can safely modify this file if you need to -- it will not be overwritten by the installer */' . "\n\n");
			fwrite($f, "\$EREGANSU_START_TIME = microtime(true);\n\n");
	...
	protected function checkPlugins()
	{
		...
		{
			$this->ui->notice(PLUGINS_ROOT . " does not exist, creating");
			mkdir(PLUGINS_ROOT);
			chmod(PLUGINS_ROOT, 0755);
		}
		else
		{
			$this->ui->notice(PLUGINS_ROOT . " already exists, leaving untouched");
		}
	}
